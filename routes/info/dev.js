...
router.get('/getdev:dtls', (req, res) => {
    ...
    let sql_fetchCurr = "Select status,dev_id,id,sno,tender_id,DATE_FORMAT(dev_dt,'%Y-%m-%d') as dev_dt,dev_type,dev_quantity,is_comp,sortype,sordesc,create_user_id,create_timestamp from " + db + ".dev where tender_id=" + SqlString.escape(obj.tender_id)
...
router.post('/createSOR', (req, res) => {
    ...
    let objectToSend = {}
    let db = "svayam_" + obj.b_acct_id + "_ebill";
    let amt=obj['rate']*obj['quantity']
    let create_timestamp = SqlString.escape(moment().format('YYYY-MM-DD HH:mm:ss'))
    let dev_dt = SqlString.escape(moment().format('YYYY-MM-DD'))
    let eff_quan=obj.quantity-obj.already_measured
    let sql_insert = "insert into " + db + ".tender_item (already_measured,sno,tender_id,unit,rate,quantity,item_cd,item_desc,item_grp_desc,"
        +"line_amt,eff_rate,eff_quantity,eff_amt,curr) values"
        + " ("+ SqlString.escape(obj.already_measured)  +","+ SqlString.escape(obj.sno) +","+ SqlString.escape(obj.tender_id) +","+ SqlString.escape(obj.unit) +","
        + SqlString.escape(obj.rate) +","+SqlString.escape(obj.quantity)+"," +SqlString.escape(obj.item_cd)+","
        + SqlString.escape(obj.item_desc) + "," + SqlString.escape(obj.item_desc) + "," + SqlString.escape(amt) + ","+ SqlString.escape(obj.rate) + ","
        + SqlString.escape(eff_quan) + "," + SqlString.escape(amt)+ "," + SqlString.escape(obj.curr) + ") "

        let sql_insert1 = "insert into " + db + ".dev (status,dev_id,sno,tender_id,dev_dt,dev_type,dev_quantity,is_comp,sortype,sordesc,create_user_id,create_timestamp) values"
        + " ("+ SqlString.escape(obj.status) +","+ SqlString.escape(obj.dev_id)+ ","+SqlString.escape(obj.sno) +","+ SqlString.escape(obj.tender_id) +","+ dev_dt +","
        + "'A'" +","+SqlString.escape(obj.quantity)+",'N',"
        + "'S'" + "," + SqlString.escape(obj.item_desc) + ","
        + SqlString.escape(obj.create_user_id) + "," + create_timestamp + ") "
