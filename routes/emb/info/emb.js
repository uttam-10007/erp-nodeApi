...
router.get('/getEmbforbill:dtls', (req, res) => {
    ...
    let sql_fetchCurr =  "SELECT id,sno,emb_no,DATE_FORMAT(measure_dt,'%Y-%m-%d') AS measure_dt,measure_desc,rate,`no`,shape,calc,quantity,"
    +" DATE_FORMAT(`create_timestamp`,'%Y-%m-%d %H:%i:%S') AS `create_timestamp`,create_user_id,update_user_id,"
    +" DATE_FORMAT(update_timestamp,'%Y-%m-%d %H:%i:%S') AS update_timestamp,tender_id,compute  FROM " + db + ".emb_item  WHERE tender_id="+SqlString.escape(obj.tender_id)+" and emb_no >"+SqlString.escape(obj.emb_no)
...
router.get('/getEmb:dtls', (req, res) => {
    ...
    let sql_fetchCurr = "Select id,tender_id,emb_no,emb_desc,create_user_id,create_timestamp,update_user_id,update_timestamp,DATE_FORMAT(emb_dt,'%Y-%m-%d') AS emb_dt,`status`,work_order_id,project_cd from " + db + ".emb_info"
