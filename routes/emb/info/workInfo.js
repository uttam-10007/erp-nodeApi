...
router.get('/getusersForsanctioned:dtls', (req, res) => {
    let objectToSend = {}
    ...
    let sql_fetchCurr = "SELECT distinct da.user_id,da.role_cd,da.zone_cd,ui.name FROM " + db + ".data_auth da JOIN " + db + ".role_info ri ON da.role_cd=ri.role_cd "
                     +"JOIN  "+propObj.svayamSystemDbName+".user_info ui ON ui.user_id=da.user_id WHERE ri.access='ACCESSALL' OR da.zone_cd="+SqlString.escape(obj.zone_cd)

    mysqlPool.query(sql_fetchCurr, function (error, results) {
        if (error) {
            console.log("Error-->routes-->info-->WorkInfo-->getusersForsanctioned--", error)
            objectToSend["error"] = true
            objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
            res.send(objectToSend);
        } else {
...
router.get('/getWorkInfo:dtls', (req, res) => {
    ...
    if(obj['work_id']!=undefined){
        sql_fetchCurr +=" where id="+SqlString.escape(obj['work_id']) 
   }
