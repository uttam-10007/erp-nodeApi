...
router.put('/updateAlreadyMeasured', (req, res) => {
    ...
    let sql = "update " + db + ".tender_item set eff_quantity = "+SqlString.escape(eff_quantity)
        +",part_rate="+SqlString.escape(obj.part_rate)+",already_measured="+SqlString.escape(obj.new_already_measured) +" where id=" + SqlString.escape(obj.id) 
...
router.put('/updaterate', (req, res) => {
    ...
    let db = "svayam_" + obj.b_acct_id + "_ebill";
    let sql = "update " + db + ".tender set over_under = "+SqlString.escape(obj.over_under)+",discount="+SqlString.escape(obj.discount)
...
router.get('/getTender:dtls', (req, res) => {
    ...
    let sql_fetchCurr = "Select *,DATE_FORMAT(agr_dt,'%Y-%m-%d') AS agr_dt from " + db + ".work_info w left join "+db+".tender t on t.work_id = w.id where t.work_id="+SqlString.escape(obj.work_id);
...
router.post('/createTender', (req, res) => {
    ...
    let sql_insert = "insert into " + db + ".tender (discount,already_emb,work_id,over_under,over_under_per,amount,type,status,create_user_id,create_timestamp) values"
        + " ("+SqlString.escape(obj.discount)+","+SqlString.escape(obj.already_emb)+","+ SqlString.escape(obj.work_id)  +","+SqlString.escape(obj.over_under) + "," + SqlString.escape(obj.over_under_rate) + "," + SqlString.escape(obj.total_amount) +","+ SqlString.escape(obj.emb_type)+"," + "'GENERATED'" +"," + SqlString.escape(obj.create_user_id) + ","
        + "" + create_timestamp + ") "
    ...
    mysqlPool.query(sql_insert, function (error, results) {
        if (error) {
            ...
            //console.log(results);
            var data = obj.data;
            var query1 = "insert into "+db+".tender_item (is_extra_item,part_rate,sno,tender_id,unit,rate,quantity,item_cd,item_desc,item_grp_desc,line_amt,line_amt_words,eff_rate,eff_quantity,eff_amt,curr,already_measured) values";
            for(var i=0;i<data.length;i++){
                var ob = data[i];
                query1+="("+SqlString.escape(ob.is_extra_item)+","+SqlString.escape(ob.part_rate)+","+SqlString.escape(ob.s_no)+","+SqlString.escape(insertId)+","+SqlString.escape(ob.unit)+","+SqlString.escape(ob.rate)+","+SqlString.escape(ob.quantity)+","+"''"+","+SqlString.escape(ob.line_desc)+","+SqlString.escape(ob.line_group_desc)+","+SqlString.escape(ob.lime_amt_without_tax)+","+SqlString.escape(ob.line_amt_in_word)+","+SqlString.escape(ob.rate)+","+SqlString.escape(ob.eff_quantity)+","+SqlString.escape(ob.lime_amt_without_tax)+","+SqlString.escape(ob.curr)+","+SqlString.escape(ob.already_measured)+")"
                if(i!=data.length-1){
                    query1+=",";
                }
            }
...
router.delete('/deleteTender:dtls', (req, res) => {
    ...
    let sql_dev = "delete from " + db + ".dev where tender_id='" + tender_id + "'"


    mysqlPool.query(sql_deleteFld+";"+sql_delete+";"+sql_dev, function (error1, results1) {
        if (error1) {
