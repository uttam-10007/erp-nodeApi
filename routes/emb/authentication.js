                            ...
                            mysqlCon.query(sql_createUser,function(error4,results4){
                                if(error4){
                                  ...
                                  //  let sql_inserRoleXref="insert into "+propObj.svayamSystemDbName+".user_xref_role (user_id,role_cd) values"
                                    ...
                                    let sql_inserRoleXref="insert into svayam_"+b_acct_id+"_ebill.data_auth (user_id,role_cd) values"
                                            +"("+user_id+",'AA')"

                                    mysqlCon.query(manualRollBack+";"+createDbQuery+";"+createTableQueries.join(";")+";"+copyTableQueries.join(";")+";"+sql_inserRoleXref,function(error5,results5){
                                        if(error5){
...
router.get('/manualloginwithotp:dtls', (req, res) => {
    ...
    let sql_getUserInfo  ="Select * from "+propObj.svayamSystemDbName+".user_info ui JOIN "+propObj.svayamSystemDbName+".billing_account ba ON ui.b_acct_id=ba.b_acct_id where ui.phone_no="+phone_no
...
router.get('/sociallogin:dtls', (req, res) => {
            ...
            let sql_getUserInfo = "Select * from "+propObj.svayamSystemDbName+".user_info ui JOIN "+propObj.svayamSystemDbName+".billing_account ba ON ui.b_acct_id=ba.b_acct_id where ui.email="+email
            mysqlPool.query(sql_getUserInfo, function (error, results) {
                if (error) {
