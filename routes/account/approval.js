...
router.post('/addapproval',(req,res)=>{
    ...
    let sql="insert into svayam_"+b_acct_id+"_md.approval (doc_type,level_of_approval,section_code,designation_code,create_user_id,create_timestamp) values "
 ...
 if (i < data.length - 1) {
    ...
    mysqlPool.query(sql,function(error,results){
        if(error){
            console.log("Error-->routes-->md-->settings-->approval-->addapproval", error)
...
router.put('/updateapproval',(req,res)=>{
    ...
    mysqlPool.query(sql,function(error,results){
        if(error){
            console.log("Error-->routes-->md-->settings-->approval-->updateapproval", error)
            objectToSend["error"] = true
            objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
            res.send(objectToSend);
        }else{
...
router.delete('/deleteapproval:dtls',(req,res)=>{
    ...
    let sql="delete from svayam_"+b_acct_id+"_md.approval where id="+SqlString.escape(id)

    mysqlPool.query(sql,function(error,results){
        if(error){
            console.log("Error-->routes-->md-->settings-->approval-->deleteapproval", error)
            objectToSend["error"] = true
            objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
            res.send(objectToSend);
        }else{
...
router.post('/addapprovalstatus',(req,res)=>{
    ...
    let sql="insert into svayam_"+b_acct_id+"_md.approval_status (remark,level_of_approval,doc_type,status,doc_local_no,create_user_id,create_timestamp) values "
...
for (let i = 0; i < data.length; i++) {
 ...
 if (i < data.length - 1) {
    ...
    mysqlPool.query(sql,function(error,results){
        if(error){
            console.log("Error-->routes-->md-->settings-->approval-->addapprovalstatus", error)
            objectToSend["error"] = true
            objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
            res.send(objectToSend);
        }else{
...
router.put('/updateapprovalstatus',(req,res)=>{
    ...
    let sql="update svayam_"+b_acct_id+"_md.approval_status set level_of_approval="+SqlString.escape(level_of_approval)+","
    ...
    mysqlPool.query(sql,function(error,results){
        if(error){
            console.log("Error-->routes-->md-->settings-->approval-->updateapprovalstatus", error)
            objectToSend["error"] = true
            objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
            res.send(objectToSend);
        }else{
...
router.delete('/deleteapprovalstatus:dtls',(req,res)=>{
    ...
    mysqlPool.query(sql,function(error,results){
        if(error){
            console.log("Error-->routes-->md-->settings-->approval-->deleteapprovalstatus", error)
            objectToSend["error"] = true
            objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
            res.send(objectToSend);
        }else{
...
router.get('/getdataofapprovalstatus:dtls', (req, res) => {
    ...
    let sql_fetchCurr = "SELECT remark,id,level_of_approval,doc_type,status,doc_local_no,create_user_id,create_timestamp,update_user_id,update_timestamp from svayam_"+b_acct_id+"_md.approval_status";
    mysqlPool.query(sql_fetchCurr, function (error, results) {
        if (error) {
            console.log("Error-->routes-->md-->settings-->approval-->getdataofapprovalstatus--", error)
            objectToSend["error"] = true
            objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
            res.send(objectToSend);
        } else {
...
router.get('/getapproval:dtls', (req, res) => {
    ...
    let sql_fetchCurr = "SELECT id,doc_type,level_of_approval,section_code,designation_code,create_user_id,create_timestamp,update_user_id,update_timestamp from svayam_"+b_acct_id+"_md.approval";
    mysqlPool.query(sql_fetchCurr, function (error, results) {
        if (error) {
            console.log("Error-->routes-->md-->settings-->approval-->getapproval--", error)
            objectToSend["error"] = true
            objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
            res.send(objectToSend);
        } else {
